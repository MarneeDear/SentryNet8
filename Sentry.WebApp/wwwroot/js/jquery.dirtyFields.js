(function($) {
	...
	$.fn.dirtyFields.configureField= function($object,$container,context,target) {
		if(!$object.hasClass($container.data("dF").exclusionClass)) {
			...
			switch(context) {
				case "text":
                    $object.on("change paste keyup", function() {
		...
		if ($container.data("dF").denoteDirtyFields) {
			...
			if (overridden === false) {
				...
				switch (updateSettingsArray[0]) {
                    ...
                    case "parent":
                        if (status === "changed") {
                            $object.parent(updateSettingsArray[1]).addClass($container.data("dF").dirtyFieldClass);
                        }
                        else {
                            $object.parent(updateSettingsArray[1]).removeClass($container.data("dF").dirtyFieldClass);
                        }
                        break;

                    case "grandParent":
                        if (status === "changed") {
                            $object.parents(updateSettingsArray[1]).closest(".form-group").addClass($container.data("dF").dirtyFieldClass);
                        }
                        else {
                            $object.parents(updateSettingsArray[1]).closest(".form-group").removeClass($container.data("dF").dirtyFieldClass);
                        }
                        break;
		...
		if (elemValue !== startingValue)
			{
                ...
                updateContext("dateTimeContext", $object, "changed", $container);
		...
		else 
			{
                ...
                updateContext("dateTimeContext", $object, "unchanged", $container);
